...
# *List* in {{< fa brands r-project title="R logo" >}}
...
# Let's practice
<div class="page-center vertical-center"> 
<p class="custom-subtitle bottombr"> Writing functiions ins the *tidyverse*</p> 
</div>


## How would we write a function that:

- creates a boxplot comparing the mean life expectancy between countries
- Name it **`country_boxplot`**

*Hint:*
```{.r code-line-numbers="5"}
country_boxplot <- function(data, .country, x = year, y = lifeExp) {
data |>
    <...>
    ggplot(aes(x = {{ x }}, y = {{ y }})) +
    <...>
...
![](assets/apple-touch-icon-152x152.png){.absolute bottom=0 right=10 width="300" height="300"}
...
![](assets/Pasted image 20240917112114.png)
...
Using map_dbl() for numerical output.
...
Using `map()` to iterate over data frame columns. Example:
