...
Purpose: R automatically converts data types in vectors. Hierarchy: Logical → Integer → Double → Character. Example: Demonstrating coercion.
...
Accessing elements: Single or multiple elements using \[\]. Named elements: Access using names. Example:

```{r}
vec <- c(alpha = 1, beta = 2, gamma = 3)
vec["beta"]  # Accessing by name
vec[c(1, 3)] # Accessing multiple elements
```

## Slide 6: Logical Indexing

Definition: Subset vectors based on conditions. Example:
...
# Section 2: List Data Structures in R

## Slide 7: Introduction to Lists

Definition: Lists are recursive vectors that can hold elements of different types. Examples: Lists within lists and different data types.

```{r}
my_list <- list(num = 1, char = "text", vec = c(1, 2, 3))
```

## Slide 8: Accessing List Elements

Single vs. Double Brackets: \[ \] vs. \[\[ \]\]. Using \$ for named elements. Example:
